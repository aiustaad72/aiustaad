...
const LANGS = [
  ...
  { id:"urdu",    label:"Only Urdu",      flag:"🇵🇰" },
  { id:"english", label:"Only English",   flag:"🇺🇸" },
  { id:"arabic",  label:"Arabic",         flag:"🇸🇦" },
  { id:"hindi",   label:"Hindi",          flag:"🇮🇳" },
];

const FEATURES = [
  { id:"tutor",  icon:"🤖", label:"AI Tutor",       desc:"Ask any question"      },
  { id:"group",  icon:"👥", label:"Group Study",     desc:"Study with friends"    },
  { id:"exam",   icon:"📝", label:"Exam Prep",       desc:"Practice with AI"      },
  { id:"tools",  icon:"🛠", label:"Study Tools",     desc:"MCQ, Notes, Planner"   },
  { id:"tips",   icon:"💡", label:"Daily Tips",      desc:"Motivational tips"     },
];

function getLang(l) {
  if(l==="urdu")    return "Sirf Urdu mein jawab do.";
  if(l==="english") return "Answer only in English.";
  if(l==="arabic")  return "أجب باللغة العربية فقط.";
      ...
      <div style={{...S.sidebar, transform:sidebar?"translateX(0)":"translateX(-100%)"}}>
        ...
        {hasBonus&&(
          <div style={{...S.sideUsage,background:"#f0fdf4",border:"1px solid #bbf7d0"}}>
            <div style={{fontSize:12,color:"#16a34a",fontWeight:600}}>🎁 Bonus Active until {new Date(user.shareBonus).toLocaleDateString()}</div>
          </div>
        )}
        ...
        <button style={S.sideItem} onClick={()=>{ setModal("lang"); setSidebar(false); }}>
          <span style={{fontSize:18}}>{LANGS.find(l=>l.id===lang)?.flag||"🌐"}</span>
          <div>
        ...
        <button style={S.sideItem} onClick={()=>{ setModal("upgrade"); setSidebar(false); }}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:PLANS[plan].color}}>{PLANS[plan].name} — {PLANS[plan].price}</div>
          </div>
        </button>
...
function UpgradeModal({plan,onUpgrade,onClose}){
  const plans=[
    {id:"free",  icon:"🆓",name:"Free",       price:"$0",        f:["10 questions/day","AI Tutor","Group Study","Daily Tips"]},
    {id:"pro",   icon:"⚡",name:"Pro",         price:"$5/month",  f:["Unlimited questions","Exam Practice","MCQ Generator","Study Planner","Translator","📸 Photo Questions"]},
...
function ShareModal({user,onUpdate,onClose,hasBonus,inCooldown,cooldownLeft}){
  ...
  const shareText=`📚 AI Ustaad - Pakistan ka best AI study app! Bilkul free try karo:\n${shareLink}`;
  ...
  const markShared=()=>{
    if(!justShared){ alert("Pehle Share ya Copy dabao — phir yahan click karo!"); return; }
    ...
    if(n>=SHARE_REQUIRED){
      ...
      alert("🎉 1 week FREE bonus mil gaya!");
      onClose();
    } else {
      ...
      <div style={S.modal}>
        ...
        {hasBonus?(
          ...
          <div style={{background:"#fef2f2",border:"1px solid #fecaca",borderRadius:14,padding:16,textAlign:"center"}}>
            ...
            <button style={{...S.btn,marginTop:12,background:"#6366f1"}} onClick={()=>{ window.location.href="#upgrade"; onClose(); }}>Ya Pro lo → $5/month</button>
          </div>
        ):(
          <>
            <div style={{background:"#fef9c3",border:"1px solid #fde68a",borderRadius:12,padding:12,textAlign:"center"}}>
              ...
              <div style={{color:"#92400e",fontSize:13,marginTop:4}}>Share with {remaining} more NEW friends → 1 week FREE!</div>
            </div>
            ...
            <button style={{...S.btn,background:justShared?"#16a34a":"#e2e8f0",color:justShared?"#fff":"#94a3b8"}} onClick={markShared}>
              {justShared?`✅ Done! (${shareCount+1}/${SHARE_REQUIRED})`:"🔒 Share First, Then Tap Here"}
            </button>
            <div style={{fontSize:11,color:"#94a3b8",textAlign:"center"}}>⚠️ Every time NEW friends required</div>
          </>
        )}
      </div>
    </div>
...
function LangModal({current,onSelect,onClose}){
  return(
    <div style={S.overlay}>
      <div style={S.modal}>
        <div style={S.mHead}>
          <span style={S.mTitle}>🌐 Language</span>
          <button style={S.closeBtn} onClick={onClose}>✕</button>
        </div>
        {LANGS.map(l=>(
          <button key={l.id} style={{display:"flex",alignItems:"center",gap:14,border:`2px solid ${current===l.id?"#f59e0b":"#f1f5f9"}`,background:current===l.id?"#fffbeb":"#fff",borderRadius:12,padding:"13px 16px",cursor:"pointer",width:"100%",marginBottom:8}} onClick={()=>onSelect(l.id)}>
            ...
            {current===l.id&&<span style={{color:"#f59e0b",marginLeft:"auto",fontWeight:700}}>✓</span>}
          </button>
        ))}
      </div>
    </div>
...
function PaymentModal({onClose}){
  const methods=[
    {icon:"💚",name:"EasyPaisa",  desc:"0300-XXXXXXX",    color:"#16a34a",status:"Coming Soon"},
    {icon:"🔴",name:"JazzCash",   desc:"0300-XXXXXXX",    color:"#dc2626",status:"Coming Soon"},
    {icon:"🔵",name:"Stripe",     desc:"Visa/Mastercard", color:"#6366f1",status:"Coming Soon"},
    {icon:"💛",name:"Payoneer",   desc:"International",   color:"#f59e0b",status:"Coming Soon"},
  ];
  return(
    <div style={S.overlay}>
      <div style={S.modal}>
        <div style={S.mHead}>
          <span style={S.mTitle}>💳 Payment Options</span>
          <button style={S.closeBtn} onClick={onClose}>✕</button>
        </div>
        <p style={{fontSize:13,color:"#64748b",margin:"0 0 12px"}}>Subscription payment methods — coming soon!</p>
        {methods.map(m=>(
        ...
        <div style={{background:"#fffbeb",border:"1px solid #fde68a",borderRadius:12,padding:12,fontSize:13,color:"#92400e",textAlign:"center",marginTop:4}}>
          💡 Abhi Pro use karne ke liye <strong>Share with 5 friends</strong> option use karo — 1 week FREE!
        </div>
      </div>
    </div>
  );
...
function TutorScreen({lang,guard,user,onUpgrade}){
  ...
  const send=async()=>{
    ...
    const userMsg={role:"user",text:q||(curImg?"📸 Photo question":""),img:curPrev};
    setMsgs(m=>[...m,userMsg]);
    setLoading(true);
    try{
      ...
      const userContent=curImg
        ...
        :q;
      const body={model:"claude-sonnet-4-20250514",max_tokens:1000,system:sys,messages:[...history,{role:"user",content:userContent}]};
  ...
  return(
    <div style={S.chatWrap}>
      ...
      <div style={{...S.subBar,justifyContent:"space-between",alignItems:"center"}}>
        ...
        {msgs.length>0&&<button onClick={clearChat} style={{background:"#fef2f2",border:"1px solid #fecaca",borderRadius:8,padding:"4px 10px",cursor:"pointer",color:"#ef4444",fontSize:12,flexShrink:0,marginLeft:6}}>🗑</button>}
      </div>
      ...
      <div style={S.chatBox}>
        ...
        {loading&&(
          <div style={{display:"flex",gap:10,padding:"8px 0"}}>
            <div style={{...S.msgAvatar,background:"#1e293b"}}>🤖</div>
            <div style={{...S.msgBubble,background:"#fff",color:"#94a3b8"}}>Thinking... ⏳</div>
          </div>
        )}
        <div ref={endRef}/>
      </div>
      ...
      {imgPrev&&(
        <div style={{padding:"6px 16px",background:"#fffbeb",borderTop:"1px solid #fde68a",display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{fontSize:12,color:"#92400e",flex:1}}>📸 Photo ready to send</span>
          <button style={{background:"#fef2f2",border:"1px solid #fecaca",borderRadius:8,padding:"4px 8px",cursor:"pointer",color:"#ef4444",fontSize:11}} onClick={()=>{setImgPrev(null);setImgB64(null);}}>✕</button>
        </div>
      )}
      ...
      <div style={S.inputWrap}>
        <div style={S.inputBox}>
          ...
          <button style={{...S.iconAction,color:isPro?"#ec4899":"#cbd5e1",position:"relative"}} onClick={()=>{if(!isPro){alert("📸 Photo feature is for Pro users only!\n\nUpgrade to Pro → $5/month");return;}fileRef.current?.click();}}>
            📸
            {!isPro&&<span style={{position:"absolute",top:-4,right:-4,background:"#f59e0b",borderRadius:"50%",width:14,height:14,fontSize:8,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:900}}>★</span>}
          </button>
          ...
          <button style={{...S.sendBtn,background:input.trim()||imgB64?"#f59e0b":"#e2e8f0",color:input.trim()||imgB64?"#fff":"#94a3b8"}} onClick={send} disabled={loading}>➤</button>
        </div>
      </div>
    </div>
  );
...
function GroupScreen({lang,guard,userName,onUpgrade}){
  ...
  const create=()=>{
    ...
    ROOMS[c]={msgs:[{from:"System",text:`🎉 Room "${c}" created! Share this code with friends.`,isAI:false}],members:[userName]};
    enter(c);
  };
  const join=()=>{
    const c=code.toUpperCase().trim(); if(!c) return;
    if(!ROOMS[c]) ROOMS[c]={msgs:[{from:"System",text:`📚 Room "${c}" is active!`,isAI:false}],members:[]};
    if(!ROOMS[c].members.includes(userName)) ROOMS[c].members.push(userName);
    ROOMS[c].msgs.push({from:"System",text:`👋 ${userName} joined!`,isAI:false});
    enter(c);
  };
  ...
  const send=async(aiMode=false)=>{
    ...
    if(aiMode||text.toLowerCase().startsWith("@ai")){
      ...
      try{
        ...
        ROOMS[myRoom].msgs.push({from:"🤖 AI Ustaad",text:reply,isAI:true});
        setMsgs([...ROOMS[myRoom].msgs]);
      }catch(e){ROOMS[myRoom].msgs.push({from:"System",text:"❌ "+e.message,isAI:false});setMsgs([...ROOMS[myRoom].msgs]);}
      setLoading(false);
    }
  };
  ...
  if(!joined) return(
    <div style={{...S.page,maxWidth:500,margin:"0 auto"}}>
      <div style={S.featureCard}>
        ...
        <div style={{fontSize:13,color:"#64748b",textAlign:"center",marginBottom:16}}>Study with friends — ask AI together!</div>
        <button style={S.btn} onClick={create}>Create New Room ✨</button>
        ...
        <button style={{...S.btn,background:"#1e293b"}} onClick={join}>Join Room →</button>
      </div>
      <div style={{background:"#fffbeb",border:"1px solid #fde68a",borderRadius:12,padding:"10px 14px",fontSize:13,color:"#92400e"}}>
        💡 Type <strong>@ai</strong> in chat to ask AI Ustaad — everyone sees the answer!
      </div>
    </div>
  );
  ...
  return(
    <div style={S.chatWrap}>
      <div style={{padding:"8px 16px",background:"#fffbeb",borderBottom:"1px solid #fde68a",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0,flexWrap:"wrap",gap:6}}>
        <span style={{fontWeight:700,color:"#92400e"}}>🏠 Room: {myRoom}</span>
      ...
      <div style={S.chatBox}>
        ...
        {loading&&<div style={{padding:"8px 16px",color:"#94a3b8",fontSize:13}}>🤖 AI is thinking...</div>}
        <div ref={endRef}/>
      </div>
      <div style={S.inputWrap}>
        <div style={S.inputBox}>
          <button style={{...S.iconAction,background:"#10b981",color:"#fff",borderRadius:10,padding:"0 12px",fontSize:13,fontWeight:600}} onClick={()=>send(true)} disabled={loading}>🤖 AI</button>
          ...
          <button style={{...S.sendBtn,background:"#f59e0b"}} onClick={()=>send()} disabled={loading}>➤</button>
        </div>
      </div>
    </div>
  );
...
function ExamScreen({lang,guard,onUpgrade}){
  ...
  const start=async()=>{
    ...
    try{
      ...
      const body={model:"claude-sonnet-4-20250514",max_tokens:1500,system:sysPrompt,messages:[{role:"user",content:userPrompt}]};
  ...
  if(step==="exam") return(
    <div style={{...S.page,gap:8}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#fffbeb",border:"1px solid #fde68a",borderRadius:12,padding:"10px 14px",position:"sticky",top:0}}>
        ...
        <span style={{color:timeLeft<60?"#ef4444":"#16a34a",fontWeight:800,fontSize:18}}>⏱ {fmt(timeLeft)}</span>
        <button style={{...S.btn,width:"auto",padding:"6px 16px",fontSize:13}} onClick={submit}>Submit ✓</button>
      </div>
      {qs.map((q,i)=>(
...
function SumTool({lang,guard,onUpgrade}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      ...
      {res&&<div style={S.resBox}><div style={{fontWeight:700,color:"#f59e0b",marginBottom:8}}>✅ Result:</div><div style={{whiteSpace:"pre-wrap",lineHeight:1.7,fontSize:14,color:"#1e293b"}}>{res}</div></div>}
    </div>
  );
}
...
function MCQTool({lang,guard,onUpgrade}){
  ...
  const go=async()=>{
    ...
    try{
      ...
      const body={model:"claude-sonnet-4-20250514",max_tokens:1200,system:sysPrompt2,messages:[{role:"user",content:userPrompt2}]};
      const text=await callAPI(body);
...
function PlanTool({lang,guard,onUpgrade}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      ...
      <button style={S.btn} onClick={go} disabled={loading}>{loading?"Creating... ⏳":"Create Study Plan 📅"}</button>
      {plan&&<div style={S.resBox}><div style={{fontWeight:700,color:"#f59e0b",marginBottom:8}}>📅 Your Plan:</div><div style={{whiteSpace:"pre-wrap",lineHeight:1.8,fontSize:14,color:"#1e293b"}}>{plan}</div></div>}
    </div>
  );
}
...
function TransTool({lang,guard,onUpgrade}){
  ...
  const go=async()=>{
    if(!text.trim()) return; if(!guard()){onUpgrade();return;} setLoading(true);
    try{ const r=await askAI({ur2en:"Translate Urdu to English. Only translation.",en2ur:"Translate English to Urdu. Only translation.",auto:"Detect and translate Urdu↔English. Only translation."}[dir],text,500); setRes(r); }
    catch(e){ setRes("❌ "+e.message); }
    setLoading(false);
  };
  return(
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      ...
      {res&&<div style={S.resBox}><div style={{fontWeight:700,color:"#f59e0b",marginBottom:8}}>✅ Translation:</div><div style={{fontSize:16,lineHeight:1.7,color:"#1e293b"}}>{res}</div></div>}
    </div>
  );
}
  ...
  return(
    <div style={{...S.page,maxWidth:500,margin:"0 auto"}}>
      <div style={S.featureCard}>
        ...
        {loading?(
          <div style={{textAlign:"center",color:"#94a3b8",padding:20}}>Getting your tip... ⏳</div>
        ):(
...
// ════════════════════════════════════════
const S={
  root:       {minHeight:"100vh",background:"#f8fafc",display:"flex",fontFamily:"'Segoe UI',Tahoma,sans-serif",position:"relative",overflow:"hidden"},
  sideOverlay:{position:"fixed",inset:0,background:"#00000055",zIndex:40},
  sidebar:    {position:"fixed",left:0,top:0,bottom:0,width:280,background:"#fff",boxShadow:"2px 0 20px #0002",zIndex:50,display:"flex",flexDirection:"column",transition:"transform 0.25s ease",overflowY:"auto"},
  ...
  inputWrap:  {padding:"12px 16px",background:"#fff",borderTop:"1px solid #f1f5f9",flexShrink:0},
